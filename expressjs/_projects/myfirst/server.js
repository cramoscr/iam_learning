...
// Tip:
//   Requires the installation of mongoose libray
...
// This handles the users entering to the main page to see
// the contents [items already stored]
app.get("/", function (req, res) {
  //const day = date.getDate();
  //Item.find({}, function (err, foundItems) {
  Item.find().then((foundItems) => {
    // If ToDo items collection is empty, populate it with initial dataset
    if (foundItems.length === 0) {
      console.log('Vamos a insertar Initial Elements');
      Item.insertMany(startItems).then((startItems) => {
        // Work needed here to handle inserting errors
        //if (err) {
        //  console.log(err);
        //} else {
...
app.post("/", function (req, res) {
  ...
  if (listName === "Today") {
    ...
    console.log('Work needed here 3');
    /*
...
app.post("/delete", function (req, res) {
  ...
  console.log('Vamos a borrar: ' + checkedItemId);
